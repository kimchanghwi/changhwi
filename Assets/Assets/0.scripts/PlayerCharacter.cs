...
public class PlayerCharacter : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            Destroy(Instantiate(JumpEffectPrefab, transform.position, Quaternion.Euler(80, 0, 0)), 2f); // 점프 이펙트 프리팹 인스턴스화)
        ...
        {
            GameObject dashEffect = Instantiate(DashEffectPrefab, transform.position, Quaternion.Euler(0, -90, 90)); // 대쉬 이펙트 프리팹 인스턴스화
            dashEffect.transform.localScale = Vector3.one * 3f;
            Destroy(dashEffect, 2f); // 대쉬 이펙트 프리팹 인스턴스화
